...
class NodeBipariteWith45VariableFeatures(ecole.observation.NodeBipartite):
    '''
    Adds (mostly global) features to variable node features.

    Adds 26 extra variable features to each variable on top of standard ecole
    NodeBipartite obs variable features (19), so each variable will have
    45 features in total.
...
#         num_feasible_best_sols_found_frac = m.getNBestSolsFound() / m.getNSolsFound()
        ...
        num_feasible_solutions_relative_to_nodes = m.getNSols() / m.getNNodes()

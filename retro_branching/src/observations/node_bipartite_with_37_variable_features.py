...
class NodeBipariteWith37VariableFeatures(ecole.observation.NodeBipartite):
    '''
    Adds 18 extra variable features to each variable on top of standard ecole
    ...
    def extract(self, model, done):
        ...
        if self.init_dual_bound is None:
            ...
            self.init_dual_bound_inverse = 1 / self.init_dual_bound # added
            self.init_primal_bound_inverse = 1 / self.init_primal_bound # added
        ...
        curr_primal_bound_inverse = 1 / m.getPrimalbound() # added
        curr_dual_bound_inverse = 1 / m.getDualbound() # added
        curr_primal_dual_bound_gap_frac = m.getGap() # added
...
# #         num_feasible_best_sols_found_frac = m.getNBestSolsFound() / m.getNSolsFound()
        ...
        best_node = m.getBestboundNode() # changed
        ...
        if m.getDepth() != 0:
            ...
            curr_node_depth = 1
        curr_node_lower_bound_inverse = 1 / curr_node.getLowerbound() # added
        ...
        curr_node_lower_bound_relative_to_curr_dual_bound =  m.getDualbound() / curr_node.getLowerbound() # added
        curr_node_lower_bound_relative_to_curr_primal_bound =  curr_node.getLowerbound() / m.getPrimalbound() # added
        ...
        feats_to_add = np.array([[
                                 self.init_dual_bound_inverse,
                                 self.init_primal_bound_inverse,
                                 curr_primal_bound_inverse,
                                 curr_dual_bound_inverse,
                                 curr_primal_dual_bound_gap_frac,
                                 num_siblings_frac,
                                 is_curr_node_best,
                                 is_curr_node_parent_best,
                                 curr_node_depth,
                                 curr_node_lower_bound_inverse,
                                 ...
                                 curr_node_lower_bound_relative_to_curr_primal_bound,
